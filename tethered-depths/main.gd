...
# Tutorial sign foundation protection (prevents mining under the sign)
@export var tutorial_sign_protect_half_width_tiles: int = 1
@export var tutorial_sign_protect_depth_tiles: int = 2
...
func _ready():
	...
	generate_world()
	position_entities()
	_register_unbreakable_under_tutorial_sign()

func _register_unbreakable_under_tutorial_sign() -> void:
	if tilemap == null:
		return
	var sign := get_node_or_null("Signtutorial") as Node2D
	if sign == null:
		return
	_register_unbreakable_rect_under_global_pos(
		sign.global_position,
		tutorial_sign_protect_half_width_tiles,
		tutorial_sign_protect_depth_tiles
	)

func _register_unbreakable_rect_under_global_pos(global_pos: Vector2, half_width_tiles: int, depth_tiles: int) -> void:
	if tilemap == null:
		return
	# Compute a probe position slightly below the object so we land inside the ground tile.
	...
	var probe_global := global_pos + Vector2(0.0, tile_step_y * 0.5)
